...
export function encodeBytes(fieldNum: number, buf: Buffer): Buffer {
	return Buffer.concat([encodeTag(fieldNum, 2), encodeVarint(buf.length), buf]);
}

export function encodeMessage(fieldNum: number, body: Buffer): Buffer {
	return encodeBytes(fieldNum, body);
}
...
export function encodePackedVarints(fieldNum: number, values: number[]): Buffer {
	const inner = Buffer.concat(values.map((v) => encodeVarint(v)));
...
export function asBuffer(value: bigint | Buffer): Buffer | null {
	return Buffer.isBuffer(value) ? value : null;
}

export function asString(value: bigint | Buffer): string | null {
...
export function asFloat32(value: bigint | Buffer): number | null {
	const buf = asBuffer(value);
	if (!buf || buf.length < 4) return null;
	const n = buf.readFloatLE(0);
