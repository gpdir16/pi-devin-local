...
import { resolveModelUid } from "./catalog.js";
...
async function* streamChatEvents(args: {
	...
	maxOutputTokens?: number;
	signal?: AbortSignal;
	onResponse?: (status: number, headers: Headers) => Promise<void> | void;
	...
	await args.onResponse?.(resp.status, resp.headers);
	if (!resp.ok) {
...
export function streamDevin(
	...
	void (async () => {
		...
		try {
			const apiKey = normalizeSessionToken(options?.apiKey);
			if (!apiKey) throw new Error("No Devin credentials. Run /login devin.");
			const host = (options?.env?.DEVIN_API_SERVER_URL || model.baseUrl || DEFAULT_API_SERVER).replace(/\/$/, "");
			...
			await options?.onPayload?.(
				{ provider: "devin", model: modelUid, tools: mapped.tools.length, messages: mapped.messages.length },
				model,
			);
			...
			for await (const event of streamChatEvents({
				...
				onResponse: async (status, headers) => {
					...
					await options?.onResponse?.({ status, headers: headerObj }, model);
				},
			})) {
				if (event.kind === "text") {
					closeThinking();
